...
export const chatWithBoard = action({
  ...
  handler: async (ctx, args) => {
    ...
    const searchResults: any[] = await ctx.runAction(api.search.semanticSearch as any, {
    ...
    searchResults.forEach((doc: any, i: number) => {
      const content = doc.scrapedContent || doc.content || doc.description || doc.title || "";
      contextStr += `\n[${i + 1}] Type: ${doc.type}\nContent: ${content}\nUrl: ${doc.url || "N/A"}\n---`;
    });
